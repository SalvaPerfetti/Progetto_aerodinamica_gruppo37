% calcolo errore variazione complessiva pannelli
...
% Impongo che la discretizzazione in corda e in apertura vari da 1 a 20

% DiscrStepChord_vect = 1:1:20;
% DiscrStepSemiSpan_vect = 1:1:20;

DiscrStepChord_vect = 1:1:40;

for itChord = 1:length(DiscrStepChord_vect)+1
...
config.AspectRatio = [7.52]; 
config.Span = [11];
config.LEPosition_X = [0];
config.LEPosition_Y = [0];
config.LEPosition_Z = [0.9];
...
if err <= toll
    NumTotPannelli=(DiscrStepChord_vect(itSpan)^2)*2;
    fprintf('La convergenza per il Cessna è raggiunta con un numero di pannelli %i \n',NumTotPannelli)
    break;
end
...
figure
grid on
plot(2.*(DiscrStepChord_vect(2:itSpan)).^2,errvect_chord,'*-')
title('Errore percentuale al variare del numero dei pannelli in corda Cessna 172')
