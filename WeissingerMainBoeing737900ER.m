...
config.RootChord = [7.88 3.8]; % [m]
config.DihedralAngle = [6 7]; % [°]
config.SweepAngle = [28.52 34.4]; % [°]
config.TaperRatio = [0.159 0.203]; %[-]
config.AspectRatio = [9.45 6.16]; %[-]
config.Span = [34.32 14.35]; %[m]
config.LEPosition_X = [0 21.05]; %[m]
config.LEPosition_Y = [0 0]; %[m]
config.LEPosition_Z = [0 2.09]; %[m]
...
config.SemiSpanwiseDiscr = [9 9];
config.ChordwiseDiscr = [9 9];
...
%% Construction of the constant term
rowIndex = 0;
for iCorpo = 1:config.NCorpi
...
% Plot of the circulation for angle of attack 2 degrees and tail
% deflection of -5,0,+5 degrees (a plot for all angles of attack is too espensive in terms of computational cost)
...
% Visualization of distributed lift along the span
% figure
% for iCorpo = 1:config.NCorpi
...
%Cycle that computes quarter-chord points for every panel strip in the
%spanwise direction
...
% Compute induced velocity by wake vortices in every quarter-chord point
...
% Computation of induced angles of attack due to the wake vortices
% (semi-infinite) in every quarter-chord point
...
for iCorpo = 1:config.NCorpi
    ...
    % Computation of the wing contribution
    if iCorpo==1
        DRAG_ala = sum(D_tot);
...
% Plot for the wing (considering tail contribution)
...
for conta=1:length(ycoda)
    ...
    xlabel('\alpha (°)')
    ylabel('C_{L}')
    title('C_{L}(\alpha) (Solo Ala, Boeing 737 900 ER)')
...
for conta=1:length(ycoda)
    ...
    xlabel('C_{D}')
    ylabel('C_{L}')
    title('Polare : C_{D}(C_{L}) (Solo Ala)')
    legend('Deflessione = -5°','Deflessione = 0°','Deflessione = +5°')
end

% Plot for all aircraft surfaces (wing+tail)
...
for conta=1:length(ycoda)
    ...
    xlabel('\alpha (°)')
    ylabel('C_{L}')
    title('C_{L}(\alpha) (Velivolo completo, Boeing 737 900 ER)')
    legend('Deflessione = -5°','Deflessione = 0°','Deflessione = +5°')
end
...
for conta=1:length(ycoda)
    ...
    xlabel('\alpha (°)')
    ylabel('C_{D}')
    title('C_{D}(\alpha) (Velivolo completo, Boeing 737 900 ER)')
    legend('Deflessione = -5°','Deflessione = 0°','Deflessione = +5°')
end
...
for conta=1:length(ycoda)
    ...
    xlabel('C_{D}')
    ylabel('C_{L}')
    title('Polare C_{D}(C_{L}) (Velivolo completo, Boeing 737 900 ER)')
    legend('Deflessione = -5° ','Deflessione = 0° ','Deflessione = +5° ')
end
